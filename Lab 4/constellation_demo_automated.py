#!/usr/bin/env python3
"""
Constellation Setup - Automated Demo Version
This script simulates joystick movements automatically for video recording
"""
...
def speak_text(text):
    ...
    if OUTPUT_MODE == 'speaker':
        # Execute text-to-speech via espeak - BLOCKING (wait for completion)
        try:
            subprocess.run(['espeak', f'"{clean_text}"'], 
...
def runAutomatedDemo(mode):
    """Run constellation setup with automated movements for video recording"""
    global OUTPUT_MODE
    OUTPUT_MODE = mode 

    print("\n--- SparkFun qwiic Joystick Astro Clicker (Automated Demo) ---\n")
    print("This demo simulates joystick movements automatically for video recording")
    print("Each movement will happen after 5 seconds...\n")
    ...
    # --- Automated Demo Sequence ---
    demo_sequence = [
        ("right", "Moving right to explore more solar system objects..."),
        ("button", "Clicking to learn about current target..."),
    ]
    
    for i, (action, description) in enumerate(demo_sequence, 1):
        print(f"\n--- Step {i}: {description} ---")
        print(f"Waiting 5 seconds before {action} movement...")
        ...
        if action == "button":
            ...
            print(f"SUCCESS: Button pressed - learned about {target['name']}")
        else:
            # Simulate joystick movement
            navigator.move(action)
            print(f"SUCCESS: Moved {action}")
        
        # No need to wait - speak_text is now blocking

# --- Entry Point ---
def main():
    # Setup argparse
    parser = argparse.ArgumentParser(
        description="Astro Clicker: Automated demo for video recording."
    )
    ...
    try:
        runAutomatedDemo(args.mode)
    except (KeyboardInterrupt, SystemExit):
        print("\nEnding Automated Demo")
        sys.exit(0)
